...
if __name__ == '__main__':
  ...
  ###The idea is to replace the prefix on each fasta file, with the NewID, to be used for the OrthoMCL analysis
    ...
    parser.add_argument("-l", "--genome_list_index", type=str,
                        help="File with the genome Taxoid, species name and the ID to replace", required=True)
    parser.add_argument("-f", "--fasta_directory", type=str,
                        help="Folder with the fasta files (downloaded from JGI)", required=True)
    parser.add_argument("-e", "--file_extension", type=str, help="Extension of the files (usually faa or fna)",
                        required=True)
    ...
    for genome in genome_dictionary:
        fasta_file = args.fasta_directory + "/" + genome + "." + args.file_extension
